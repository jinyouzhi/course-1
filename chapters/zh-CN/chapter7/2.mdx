...
- **命名实体识别 (NER)**: 找出句子中的实体（如人物、地点或组织）。这可以表述为：通过为每个实体或“无实体”指定一个类别的方式，为句中的每个 Token 分配标签。
- **词性标注 (POS)**: 将句子中的每个单词标记为对应于特定的词性（如名词、动词、形容词等）。
- **组块分析（chunking）**: 找到属于同一实体的一组 Token。这个任务（可结合 POS 或 NER ）可以任何将一组 Token 作为制定一个标签（通常是 `B-`），另一个标签（通常 `I-`） 表示 Token 是否是同一块，和第三个标签（通常是 `O`）表示 Token 不属于任何块。也就是标出句子中的短语块，例如名词短语（NP），动词短语（VP）等。
